...
// Time stamp storing the precise moment when a user pokes the jelly
float gLastPokeTime = -100.0f;
...
void create_scene()
{
    // Bypasses re-allocation to safely retain mesh source vectors over real-time ticks
    if (gVertexBuffer != nullptr) return;
...
void render()
{
    ...
    // Track dynamic timeline delta for interaction decay math
    float timeSincePoke = time - gLastPokeTime;
    ...
    for (int i = 0; i < gNumTriangles; ++i) {
        ...
        for (int j = 0; j < 3; ++j) {
            ...
            // Compute persistent, subtle background jelly ripple calculations
            float baseWave = std::sin(localPos.y * 4.0f + time * 3.0f) * 0.06f;
            float baseSlope = std::cos(localPos.y * 4.0f + time * 3.0f) * 4.0f * 0.06f;

            // Initialize impulse ripple parameter allocations
            float pokeWave = 0.0f;
            float pokeSlope = 0.0f;

            // Process user input decay physics over an active 3.5s window
            if (timeSincePoke >= 0.0f && timeSincePoke < 3.5f) {
                // Compute exponential decay envelope factor (e^-1.2t)
                float damping = std::exp(-1.2f * timeSincePoke);

                // Compute high-frequency structural impulse wave equations
                pokeWave = damping * std::sin(localPos.y * 22.0f - timeSincePoke * 28.0f) * 0.35f;

                // Evaluate standard calculus derivatives to correct normal tracking under stress
                pokeSlope = damping * std::cos(localPos.y * 22.0f - timeSincePoke * 28.0f) * 22.0f * 0.35f;
            }

            // Combine base structural vibrations and spatial poke values together
            float totalWave = baseWave + pokeWave;
            float totalSlope = baseSlope + pokeSlope;

            // Re-map vertex positions and calculate distorted normal matrices accordingly
            vec3 deformedPos = localPos + localNormal * totalWave;
            vec3 deformedNormal = glm::normalize(localNormal + vec3(totalSlope * localPos.x, 0.0f, totalSlope * localPos.z) * 0.4f);

            // Convert coordinates to World Space
            world_pos[j] = vec3(M_model * vec4(deformedPos, 1.0f));
            world_nrm[j] = glm::normalize(mat3(M_model) * deformedNormal);
        ...
        // Dispatch screen coordinates along with world parameters into the software rasterizer stage
...
int main(int argc, char* argv[])
{
    ...
    GLFWwindow* window = glfwCreateWindow(Width, Height, "Jelly Poke Interaction (Q4)", NULL, NULL);
    if (!window) {
        glfwTerminate();
        return -1;
    }
    ...
    // Render interactive user guidance layout to terminal output stream
    std::cout << "====================================================" << std::endl;
    std::cout << "  Instructions: Press the SPACEBAR to poke the jelly! " << std::endl;
    std::cout << "  Press 'Q' or 'ESC' to quit the application.       " << std::endl;
    std::cout << "====================================================" << std::endl;

    // Track state variations to form clean debounced trigger clicks
    bool spacePressedLastFrame = false;

    // Real-time animation runtime viewport graphics execution loop
    while (!glfwWindowShouldClose(window))
    {
        // Poll and debounce runtime spacebar inputs to log timestamp deltas accurately
        if (glfwGetKey(window, GLFW_KEY_SPACE) == GLFW_PRESS) {
            if (!spacePressedLastFrame) {
                gLastPokeTime = (float)glfwGetTime();
        ...
        // Run continuous, time-dependent frame re-rasterization passes
        render();
