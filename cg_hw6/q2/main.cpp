...
// Draw a Gouraud-shaded triangle by interpolating color attributes across fragments
void rasterizeGouraudTriangle(const vec3& v0, const vec3& v1, const vec3& v2,
        ...
        for (int x = minX; x <= maxX; ++x) {
            ...
            if ((w0 >= 0 && w1 >= 0 && w2 >= 0) || (w0 <= 0 && w1 <= 0 && w2 <= 0)) {
                ...
                if (z < DepthBuffer[idx]) {
                    ...
                    // Linearly interpolate pre-computed vertex colors in linear space
                    vec3 color = w0 * c0 + w1 * c1 + w2 * c2;

                    // Execute Gamma Correction (Gamma = 2.2) prior to framebuffer output
...
// -------------------------------------------------
// Vertex Shader (Gouraud / Per-Vertex Shading)
// -------------------------------------------------
vec3 computeVertexColor(const vec3& worldPos, const vec3& worldNormal, const vec3& eyePos, const vec3& lightPos) {
    ...
    // Return the uncorrected lighting value (linear color space)
    return ka * Ia + kd * Il * n_dot_l + ks * Il * specular;
}
...
void render()
{
    ...
    for (int i = 0; i < gNumTriangles; ++i) {
        ...
        for (int j = 0; j < 3; ++j) {
            ...
            world_v[j] = vec3(M_model * vec4(localPos, 1.0f));

            // Transform normals to World Space (Safe to use model matrix due to uniform scaling)
            vec3 worldNormal = glm::normalize(mat3(M_model) * localNormal);

            // --- A. Gouraud Shading Calculations ---
            // Compute lighting evaluations per vertex in world space coordinates
            vertex_colors[j] = computeVertexColor(world_v[j], worldNormal, eyePos, lightPos);

            // --- B. Project and Transform Vertices to Screen Space ---
            vec4 clipPos = VP * vec4(world_v[j], 1.0f);
        ...
        // --- C. Back-Face Culling Geometry Test ---
        vec3 faceNormal = glm::normalize(glm::cross(world_v[1] - world_v[0], world_v[2] - world_v[0]));
        vec3 viewDir = glm::normalize(eyePos - world_v[0]);
        if (glm::dot(faceNormal, viewDir) < 0.0f) continue;

        // Dispatch screen coordinates and pre-computed vertex color arrays to the rasterizer
        rasterizeGouraudTriangle(screen_v[0], screen_v[1], screen_v[2],
...
int main(int argc, char* argv[])
{
    ...
    GLFWwindow* window = glfwCreateWindow(Width, Height, "Gouraud Shading Viewer", NULL, NULL);
    if (!window) {
        glfwTerminate();
        return -1;
    }
